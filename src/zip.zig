...
// The valid range of MS-DOS dates is January 1, 1980, to December 31, 2099
...
pub fn ctime2dos(t: time.time_t, dos_date: *u16, dos_time: *u16) !void {
    ...
    // cannot store a date after the year 2099
    if (tm.tm_year - 80 > 119) {
        dos_time.* = 0;
        dos_time.* |= @as(u16, 58); // Second: 58.
        dos_time.* |= @as(u16, 59) << 5; // Minute: 59.
        dos_time.* |= @as(u16, 23) << 11; // Hour: 23.
        dos_date.* = 0;
        dos_date.* |= 31; // Day: 01 (31).
        dos_date.* |= 12 << 5; // Month: Dec (12).
        dos_date.* |= 119 << 9; // Year: 2099 (119).
        return;
    }
